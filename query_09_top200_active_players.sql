-- Top 200 Active Players
--
-- Player Address: Unique wallet address of each player.
--
-- Games: Number of new games initiated across all game modes.
--
-- Deaths (Bmode): Total number of deaths in Beast Mode.
--
-- Rank (Bmode): Best rank achieved in Beast Mode leaderboard. "-" if not ranked.
--
-- Beasts: Total number of beasts claimed by the player (calculated from claim events).
--
-- BH Rank: Player's rank on the Top Beast Hunters leaderboard (last 2 weeks). "-" if not in top 50.
--
-- Avg Lvl: Average level reached at death in Beast Mode (calculated from all deaths).
--
-- LH Rank: Player's rank on the Top Level Hunters leaderboard (last 2 weeks). "-" if not in top 50.
--
-- Total In-Game Txs: All game-related transactions including gameplay actions.
--
-- Total Paid Fees (USD): Cumulative transaction fees paid in USD.
--
-- # of Active Days: Total number of distinct days the player was active.
