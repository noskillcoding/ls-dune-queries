...
beast_hunters AS (
  ...
  FROM dune.lowskillcoding.result_ls_2_top_200_beast_hunters GROUP BY CAST("Player address" AS VARCHAR)
),

beast_mode_ranks AS (
  SELECT CAST("Player address" AS VARCHAR) AS player_address, MIN("Rank") AS rank
  FROM dune.lowskillcoding.result_ls_2_top_200_beast_mode GROUP BY CAST("Player address" AS VARCHAR)
),
