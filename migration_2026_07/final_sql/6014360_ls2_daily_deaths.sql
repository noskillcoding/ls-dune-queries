...
-- https://dune.com/queries/6014360
...
v1_final AS (
  SELECT
    DATE_TRUNC('day', d.block_time) AS death_date,
    CASE WHEN COALESCE(s.mode_flag, 0) = 0 THEN 'Beast Mode' ELSE 'Other' END AS bucket
...
v2_final AS (
  SELECT
    DATE_TRUNC('day', d.block_time) AS death_date,
    CASE WHEN g.dungeon = 'Beast Mode' THEN 'Beast Mode' ELSE 'Other' END AS bucket
  FROM v2_death_events d
  LEFT JOIN v2_game_dungeon g ON g.adventurer_id = d.adventurer_id
  WHERE d.rn = 1
),

all_deaths AS (
  SELECT death_date, bucket FROM v1_final
  UNION ALL
  SELECT death_date, bucket FROM v2_final
)

SELECT
  death_date AS "Date",
  COUNT(*) AS "Total deaths",
  SUM(CASE WHEN bucket = 'Beast Mode' THEN 1 ELSE 0 END) AS "Deaths in Beast Mode",
  SUM(CASE WHEN bucket = 'Other' THEN 1 ELSE 0 END) AS "Deaths in Other Modes"
FROM all_deaths
GROUP BY death_date
ORDER BY death_date
