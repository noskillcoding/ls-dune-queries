...
-- https://dune.com/queries/6014208
-- version 4 as of 2026-07-26

-- Deaths by Type (Beast Mode) -- spans the V1 -> V2 cutover
--
-- Death type is read from the adventurer's beast_health at the moment of death:
--   beast_health = 0  -> killed by an Obstacle
--   beast_health > 0  -> killed by a Beast (the beast was still alive)
--
-- 2026-07-26: added the V2 half. V1 read pgWorld Dojo events (beast_health = data[9]);
-- V2 emits its own GameEvent where beast_health is at data[5] (+1 in the V2a layout).
-- Both V2 layouts are handled -- see V2_EVENT_DECODE_REFERENCE.md.
...
all_final AS (
  SELECT beast_health FROM v1_final
  UNION ALL
  SELECT beast_health FROM v2_final
),
...
death_types AS (
  SELECT
    CASE WHEN beast_health = 0 THEN 'Obstacle'
         ...
         ELSE 'Unknown' END AS death_type,
    CASE WHEN beast_health = 0 THEN 1
         WHEN beast_health > 0 THEN 2
         ELSE 3 END AS sort_order
  FROM all_final
),

total_deaths AS (SELECT COUNT(*) AS total FROM death_types)
